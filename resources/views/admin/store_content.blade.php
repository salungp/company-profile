...
@section('content')
	<div class="box">
		<form action="{{ route('contents.store') }}" method="POST" enctype="multipart/form-data">
			{{ csrf_field() }}
			<h1 class="medium-title">Input content</h1>
			<label for="title">Judul</label>
			<input type="text" id="title" name="title" class="form-control @error('title') is-invalid @enderror" placeholder="Judul content">
			@error('title')
				<div class="small-title" style="color: #dc3545;">{{ $message }}</div>
			@enderror
			<label for="description">Description</label>
			<textarea name="description" class="form-control @error('title') is-invalid @enderror" id="description" placeholder="Masukkan description"></textarea>
			@error('description')
				<div class="small-title" style="color: #dc3545;">{{ $message }}</div>
			@enderror
			<label for="icon">Icon</label>
			<textarea name="icon" id="icon" class="form-control" placeholder="Masukkan icon"></textarea>
